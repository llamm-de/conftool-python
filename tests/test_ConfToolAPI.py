...
from conftool.ConfToolAPI import ConfToolAPI
...
def test_initialize_nonce():
    api = ConfToolAPI(ENDPOINT_DUMMY, API_KEY_DUMMY)
    api_initial_nonce = ConfToolAPI(ENDPOINT_DUMMY, API_KEY_DUMMY, initial_nonce=INITIAL_NONE_DUMMY)

    assert api.last_nonce <= int(time.time() * 10000)
    assert api_initial_nonce.last_nonce == INITIAL_NONE_DUMMY

def test_passhash():
    api = ConfToolAPI(ENDPOINT_DUMMY, API_KEY_DUMMY, initial_nonce=INITIAL_NONE_DUMMY)    
